...
2. Install the Giscus GitHub App for that repository at `https://github.com/apps/giscus`.
3. Open `https://giscus.app` and enter the same `owner/repo`.
4. Choose `pathname` mapping. This matches GitHub Pages paths such as `/marktl/s/<short-id>/`.
5. Choose a discussion category such as `General` or `Announcements`.
6. Copy the generated `data-repo-id` and `data-category-id` values from the script block.
7. Copy `repo`, `repo-id`, `category`, and `category-id` into MarkTL settings.
8. Set `Reader feedback mode` to `Giscus GitHub comments`.
9. Export with `Trusted preview/export`.

Giscus loads an external script from `https://giscus.app/client.js`, so it is intentionally disabled for sanitized exports.

For `reallygood83/moondoc`, the values look like this after Discussions and the Giscus App are enabled:

```text
Giscus repository: reallygood83/moondoc
Giscus repository ID: R_kgDOSZNBng
Giscus category: General
Giscus category ID: DIC_kwDOSZNBns4C81kX
Giscus mapping: pathname
Giscus theme: preferred_color_scheme
```

If `giscus.app` shows "giscus cannot be used", check these in order:

1. The repository is public.
2. GitHub Discussions is enabled in the repository.
3. A discussion category exists.
4. The Giscus GitHub App is installed for that exact repository.
5. You selected an existing category in `giscus.app`.

Already-published HTML does not update itself after you change Giscus settings. Re-export the note with `GitHub Pages link` selected so MarkTL can inject the Giscus script into the HTML.
...
Use a fine-grained token, not a broad classic token:

1. Open `https://github.com/settings/personal-access-tokens/new`.
2. Set a clear token name, for example `MarkTL Pages publisher`.
3. Set an expiration date.
4. Under `Repository access`, choose `Only select repositories`.
5. Select only the repository MarkTL publishes to, for example `reallygood83/moondoc`.
6. Under `Repository permissions`, grant `Contents` → `Read and write`.
7. Leave other permissions unset unless you know you need them.
8. Generate the token and paste it into `GitHub token` in MarkTL settings.

MarkTL stores this token in the Obsidian plugin data file. Keep the token narrow, rotate it if it appears on screen during a recording, and avoid using a token that can access unrelated repositories.
...
2. `https://github.com/apps/giscus`에서 해당 repo에 Giscus GitHub App을 설치합니다.
3. `https://giscus.app`을 열고 같은 `owner/repo`를 입력합니다.
4. Page ↔ Discussion mapping은 `pathname`을 선택합니다. GitHub Pages 경로 `/marktl/s/<short-id>/`와 잘 맞습니다.
5. Discussion category는 `General` 또는 `Announcements`처럼 실제 존재하는 category를 선택합니다.
6. 생성된 script block에서 `data-repo-id`, `data-category-id` 값을 복사합니다.
7. `repo`, `repo-id`, `category`, `category-id` 값을 MarkTL 설정에 입력합니다.
8. `Reader feedback mode`를 `Giscus GitHub comments`로 설정합니다.
9. `Trusted preview/export`로 export합니다.

Giscus는 `https://giscus.app/client.js` 외부 스크립트를 로드하므로 sanitized export에서는 의도적으로 비활성화됩니다.

`reallygood83/moondoc` 기준으로 Discussions와 Giscus App이 설정된 뒤 값은 아래처럼 입력합니다.
...
`giscus.app`에서 "giscus를 사용할 수 없습니다"가 나오면 아래 순서로 확인하세요.

1. repo가 public인지 확인합니다.
2. repo에서 GitHub Discussions가 켜져 있는지 확인합니다.
3. Discussion category가 실제로 존재하는지 확인합니다.
4. Giscus GitHub App이 정확히 그 repo에 설치되어 있는지 확인합니다.
5. `giscus.app`에서 존재하는 category를 선택했는지 확인합니다.

이미 발행한 HTML은 Giscus 설정 변경 후 자동으로 바뀌지 않습니다. `GitHub Pages link`로 같은 노트를 다시 export해야 MarkTL이 HTML 안에 Giscus script를 삽입합니다.
...
넓은 권한의 classic token보다 fine-grained token을 권장합니다.

1. `https://github.com/settings/personal-access-tokens/new`를 엽니다.
2. Token name을 알아보기 쉽게 입력합니다. 예: `MarkTL Pages publisher`
3. Expiration을 설정합니다.
4. `Repository access`에서 `Only select repositories`를 선택합니다.
5. MarkTL이 publish할 repo만 선택합니다. 예: `reallygood83/moondoc`
6. `Repository permissions`에서 `Contents` → `Read and write`를 부여합니다.
7. 다른 권한은 필요하지 않으면 비워둡니다.
8. token을 생성한 뒤 MarkTL settings의 `GitHub token`에 붙여넣습니다.

MarkTL은 이 token을 Obsidian plugin data file에 저장합니다. 녹화 중 token이 화면에 노출되었다면 즉시 revoke/rotate하고, unrelated repo에 접근 가능한 token은 사용하지 마세요.
